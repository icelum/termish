...
class HerdrSnapshotRealDataTest {
    ...
    private fun realSnapshot(): HerdrSessionSnapshot {
        val raw = HerdrSnapshotRealDataTest::class.java.getResourceAsStream("/herdr-snapshot.json")
    ...
    fun realSnapshotParses() {
        ...
        assertEquals(s.agents.size, s.agents.map { it.paneId }.toSet().size, "agent pane_id 应唯一")
    }
...
class HerdrMonitorTest {
    ...
    private fun snapshotWith(vararg statuses: Pair<String, HerdrAgentStatus>): String {
        val agents = statuses.joinToString(",") { (pane, st) ->
    ...
    fun pollsAndEmitsEvents() = kotlinx.coroutines.test.runTest {
        ...
        val monitor = HerdrMonitor(
            ...
            runCommand = { cmd ->
                ...
                if (round == 1) snapshotWith("w1:p1" to HerdrAgentStatus.WORKING)
                else snapshotWith("w1:p1" to HerdrAgentStatus.BLOCKED)
            },
            scope = this,
    ...
    fun blockedNotificationText() {
        val monitor = HerdrMonitor("nas", "h1", { null }, kotlinx.coroutines.CoroutineScope(kotlinx.coroutines.Dispatchers.Unconfined))
        val a = HerdrAgentInfo(
            paneId = "w1:p1",
            agent = "codex",
    ...
    fun failureBackoffThenPause() = kotlinx.coroutines.test.runTest {
        ...
        val monitor = HerdrMonitor(
            hostName = "nas",
            hostId = "h1",
            runCommand = { cmd -> calls += cmd; null }, // 永远失败（herdr 未装）
            scope = this,
            pollIntervalMs = 1_000,
        )
        monitor.start()
