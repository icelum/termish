...
- 格式：每个用例 `### <组字母><序号> <场景名>`，字段：前置/步骤/预期/恢复
  （模板见 ai-adb-gen；**无独立「依赖」字段**——用例间依赖靠组内顺序 +
  「前置」标注体现，与 gen 保持一致，不自造新字段）
...
3. 逐用例执行；**前置未满足（本用例「前置」要求的环境/一次性配置未就绪，
   或按组内顺序其前置用例 SKIP/FAIL）→ 本用例 SKIP**（注明原因）
